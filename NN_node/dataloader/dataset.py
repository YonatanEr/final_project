...
class SemKITTI(data.Dataset):
    def __init__(self, data_path, imageset = 'train', return_ref = False):
        ...
        for i_folder in split:
            ...
            self.im_idx += absoluteFilePaths("/home/eeproj1/PolarSeg/data/dataset/sequences/11/velodyne")
        self.im_idx.sort()
        print("!!!!!!!!!!")
        print(self.im_idx)
        print("!!!!!!!!!!")
...
class spherical_dataset(data.Dataset):
  ...
  def __getitem__(self, index):
        ...
        if (intervals==0).any(): print("Zero interval!")
        grid_ind = (np.floor((np.clip(xyz_pol,min_bound,max_bound)-min_bound)/intervals)).astype(np.int64)#change this to int64 because of numpy error 
